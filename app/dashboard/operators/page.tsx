      ...
      <div className="max-w-[1400px] mx-auto space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-white p-4 rounded-xl shadow-sm border-b-4 border-blue-500">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-black italic uppercase tracking-tighter">Operadores AMEL</h1>
          </div>
          <Button onClick={() => { setForm(initialFormState); setOperadorAEditar(null); setShowNew(true); }} style={{backgroundColor: AMEL_YELLOW, color: '#000'}} className="font-black italic px-6 h-10 shadow-md">
            <PlusCircle className="mr-2 h-4 w-4" /> NUEVO OPERADOR
          </Button>
        </div>
        ...
        <Card className="border-none shadow-2xl bg-white rounded-2xl overflow-hidden">
          <div className="p-4 bg-zinc-900 text-white flex items-center justify-between border-b-2 border-blue-500">
            <h3 className="font-black uppercase italic text-sm tracking-widest">Listado de Operadores ({filtrados.length})</h3>
          </div>
            ...
            <Table className="w-full">
              <TableHeader className="bg-zinc-50 border-b font-black text-xs uppercase text-zinc-600">
                <TableRow className="h-12">
                  ...
                  <TableHead className="text-left px-4">Vigencia Contrato</TableHead>
                  <TableHead className="text-left px-4">Teléfono</TableHead>
                  <TableHead className="text-left px-4">Email</TableHead>
              ...
              <TableBody>
                {filtrados.map((operador) => (
                  <TableRow key={operador.id} className="border-b hover:bg-zinc-50 transition-colors">
                    ...
                    <TableCell className="px-4 py-3 text-xs">
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-zinc-600">{operador.vigencia_contrato ? new Date(operador.vigencia_contrato).toLocaleDateString() : '---'}</span>
                        {operador.vigencia_contrato ? (() => { const vs = getVigenciaStatus(operador.vigencia_contrato); return (<span className={`${vs.color} px-2 py-1 rounded-full font-bold text-[11px]`}>{vs.label === 'Vencido' ? 'Vencido' : vs.label}</span>); })() : <span className="text-zinc-400">—</span>}
                      </div>
                    </TableCell>
                    <TableCell className="px-4 py-3 text-xs text-zinc-600">{operador.telefono || '---'}</TableCell>
                    <TableCell className="px-4 py-3 text-xs text-zinc-600">{operador.email || '---'}</TableCell>
        ...
        <DialogContent className="max-w-[95vw] lg:max-w-[1400px] bg-white rounded-3xl p-0 overflow-hidden shadow-2xl border-none">
          <div className="bg-blue-600 p-4 text-white flex items-center gap-2 border-b-4 border-blue-700">
            ...
            <h2 className="text-2xl font-black uppercase italic tracking-tighter">
              {operadorAEditar ? 'Editar Operador' : 'Nuevo Operador'}
            </h2>
          </div>
          ...
          <div className="p-4 space-y-3 max-h-[520px] overflow-y-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div>
                  <label className="text-xs font-bold text-zinc-600 uppercase mb-2 block">Email</label>
                  <Input type="email" placeholder="Email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})} className="h-11" />
                </div>
              </div>
            </div>
